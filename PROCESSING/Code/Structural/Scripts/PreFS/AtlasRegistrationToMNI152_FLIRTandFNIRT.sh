...
Usage() {
  ...
  echo "                --costmaskref=<binary cost function mask>"
}
...
# Linear then non-linear registration to MNI using cost function masking if provided


# if [ ! -z ${Lesion} ] ; then
if [[ -f "$( dirname ${WD})/lesion/T1w_lesion_mask_invert_acpc.nii.gz" ]] ; then
  CFMask="$( dirname ${WD})/lesion/T1w_lesion_mask_invert_acpc.nii.gz"
  MNIMask="$( dirname ${WD})/lesion/MNI_lesion_mask_invert.nii.gz"
  log_Msg "UPDATE:    performing Atlas registration including cost function masking"
  ${FSLDIR}/bin/flirt -interp spline -dof 12 \
    -in ${T1wRestoreBrain} \
    -inweight ${CFMask} \
    -ref ${ReferenceBrain} \
    -refweight ${MNIMask} \
    -omat ${WD}/xfms/acpc2MNILinear.mat \
    -out ${WD}/xfms/${T1wRestoreBrainBasename}_to_MNILinear

  
  ${FSLDIR}/bin/fnirt \
    --in=${T1wRestore} \
    --ref=${Reference2mm} \
    --aff=${WD}/xfms/acpc2MNILinear.mat \
    --refmask=${Reference2mmMask} \
    --inmask=${CFMask} \
    --fout=${OutputTransform} \
    --jout=${WD}/xfms/NonlinearRegJacobians.nii.gz \
    --refout=${WD}/xfms/IntensityModulatedT1.nii.gz \
    --iout=${WD}/xfms/2mmReg.nii.gz \
    --logout=${WD}/xfms/NonlinearReg.txt \
    --intout=${WD}/xfms/NonlinearIntensities.nii.gz \
    --cout=${WD}/xfms/NonlinearReg.nii.gz \
    --config=${FNIRTConfig}
else 
  log_Msg "UPDATE:    performing Atlas registration without cost function masking"

...
Usage() {
  ...
  echo " "
  echo "Usage: $(basename $0) --workingdir=<working dir> --in=<input image> --ref=<reference image> --out=<output image> --omat=<output matrix> [--brainsize=<brainsize>] --costmask=<binary cost function mask>"
}
...
CostMask=`getopt1 "--costmask" $@` #"$7"
RefMask=`getopt1 "--refmask" $@` #"$8"
...
# Crop the FOV
${FSLDIR}/bin/robustfov -i "$Input" \
  -m "$WD"/roi2full.mat \
  -r "$WD"/robustroi.nii.gz \
  $BrainSizeOpt
...
if [[ -f ${CostMask} ]] ; then
  CFMask=${CostMask%.nii.gz}"_invert.nii.gz"
  MNIMask=$( dirname ${CostMask})"/MNI_lesion_mask_invert.nii.gz"
  log_Msg "UPDATE:    running ACPCA alignment including cost function masking"
  #map lesionmask to robust field of view space
  ${FSLDIR}/bin/robustfov -i ${CFMask} \
  -m "$WD"/roi2fullmask.mat \
  -r ${CFMask%.nii.gz}"_fov.nii.gz" \
  $BrainSizeOpt

  ${FSLDIR}/bin/flirt -interp spline \
    -in "$WD"/robustroi.nii.gz \
    -inweight ${CFMask%.nii.gz}"_fov.nii.gz" \
    -ref "$Reference" \
    -refweight "${MNIMask}" \
    -omat "$WD"/roi2std.mat \
  ...
  ${FSLDIR}/bin/flirt -applyxfm -usesqform \
    ...
    -init "$WD"/roi2std.mat \
    -in ${CFMask%.nii.gz}"_fov.nii.gz" \
    -ref "$Reference" \
    -out ${CFMask%.nii.gz}"_acpc.nii.gz"

else 
  ${FSLDIR}/bin/flirt -interp spline -in "$WD"/robustroi.nii.gz -ref "$Reference" -omat "$WD"/roi2std.mat -out "$WD"/acpc_final.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30 #-refweight ${RefMask}
fi

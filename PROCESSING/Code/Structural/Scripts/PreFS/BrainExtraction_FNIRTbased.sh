...
Usage() {
  ...
  echo " "
  echo "Usage: $(basename $0) [--workingdir=<working dir>] --in=<input image> [--ref=<reference highres image>] [--refmask=<reference brain mask>] [--ref2mm=<reference image 2mm>] [--ref2mmmask=<reference brain mask 2mm>] --outbrain=<output brain extracted image> --outbrainmask=<output brain mask> [--fnirtconfig=<fnirt config file>] --costmaskref=<binary cost function mask>"
}
...
if [[ -f ${CostMask%.nii.gz}"_invert_acpc.nii.gz" ]] ; then
# if [ ! -z ${CostMask} ] ; then
  CFMask=${CostMask%.nii.gz}"_invert_acpc.nii.gz"
  MNIMask=$( dirname ${CostMask})"/MNI2mm_lesion_mask_invert.nii.gz"

  log_Msg "UPDATE:    performing brain extraction including cost function masking"
  ...
  -out "$WD"/"$BaseName"_to_MNI_roughlin.nii.gz -nosearch 
  ...
  log_Msg "performing registration without cost function masking"

...
# ## LEAPP UPDATES
#
# * Patrik Bey, Brain simulation section, Berlin Institute of Health at Charité, Department of Neurology,
#   Charité Univeristätsmedizin Berlin, Berlin ,Germany
# * 2022.10.27
#
# ## Product
#
...
LEAPP - adjusted PreFreeSurferPipeline.sh
...
Usage: PreeFreeSurferPipeline.sh [options]
  ...
  --subject=<subject>  Subject ID (required)
					   ...
					   directory for all outputs generated as path/subject
  --session=<session>  Session ID (required)
					   Used with --path and --subject to create WD directory
  ...
  --flair=<Flair image>			    anatomical FLAIR image
  ...
  --vbtsmoothing=<integer>          Sigma parameter for FWHM Gaussian Kernel during VBT.
                                    (default=2)
  --maskspace=<string>				Base modality space for lesion mask segmentation
									(default="T1w")
  --lesioncorrect=<boolean>			If variable set, perform lesion correction, else leave empty.
									(default="")
...
MaskSpace=`opts_GetOpt1 "--maskspace" $@` # space for lesion mask base image
VBTSmoothing=`opts_GetOpt1 "--vbtsmoothing" $@` # sigma value for gaussian smoothing in VBT.
LesionCorrect=`opts_GetOpt1 "--lesioncorrect" $@` # boolean wether to include lesion processing
...
log_Msg "Mask Space: ${MaskSpace}"
log_Msg "LesionCorrect: ${LesionCorrect}"
...
log_Msg "LEAPP_STRUCTDIR: ${LEAPP_STRUCTDIR}"
...
if [ -z $BrainSize ] ; then
	BrainSize=150
	log_Msg "Setting BrainSize parameter to human default value of 150 mm"
fi
...
# Create cost function mask for various modalities using the provided lesion mask in $MaskSpace.
# 

if [[ ! -z ${LesionCorrect} ]]; then
	#
	# CURRENTLY NOT INCLUDED. ADD AFTER VALIDATION OF CFM IN LARGER CONTEXT FOLLOWING BEY ET AL. 2022 FENS FORUM POSTER
	#
	#
	# if [[ ! -f "${StudyFolder}/${Subject}/anat/${MaskSpace}_lesion_mask.nii.gz" ]]; then
	# 	log_Msg "UPDATE:    No lesion mask found. Assuming healthy control; creating fake lesion mask."
	# 	python3 ${LEAPP_STRUCTDIR}/Scripts/GetFakeLesionMasks.py \
	# 		--path=${StudyFolder} \
	# 		--subject=${Subject} \
	# 		--maskspace=${MaskSpace} \
	# 		--templatedir=${HCPPIPEDIR_Templates}
	# else
	# if [[ -f "${StudyFolder}/${Subject}/anat/${MaskSpace}_lesion_mask.nii.gz" ]] ; then
		${LEAPP_STRUCTDIR}/Scripts/CreateCostMasks.sh \
			--studyfolder=${StudyFolder} \
			--subject=${Subject} \
			--session=${Session} \
			--t1winput=${T1wInputImages} \
			--t2winput=${T2wInputImages} \
			--templateinput=${T1wTemplate} \
			--templateinput2mm=${T1wTemplate2mm} \
			--flairinput=${FLAIRInput} \
			--maskspace=${MaskSpace}
	# fi
	T1wCostMaskRef="${StudyFolder}/${Subject}/${Session}/lesion/T1w_lesion_mask_invert.nii.gz"
	CostMaskMNI="${StudyFolder}/${Subject}/${Session}/lesion/MNI_lesion_mask_invert.nii.gz"
	T2wCostMaskRef="${StudyFolder}/${Subject}/${Session}/lesion/T2w_lesion_mask_invert.nii.gz"
...
#  - perform Virtual Brain Transplant of lesioned brain tissue using corresponding lesion mask
...
for TXw in ${Modalities} ; do
	...
	######################################################################
	# PERFORMING VIRTUAL BRAIN TRANSPLANT

    TXwCostMask="${StudyFolder}/${Subject}/${Session}/lesion/${TXw}_lesion_mask.nii.gz"

    if [[ -f ${TXwCostMask} ]] ; then
        # Perform Virtual Brain Transplant
        ${RUN} ${LEAPP_STRUCTDIR}/Scripts/VirtualBrainTransplant.sh \
            --workingdir=${StudyFolder} \
            --subject=${Subject} \
			--session=${Session} \
            --in=${TXwInputImages} \
            --costmask=${TXwCostMask} \
			--smoothing=${SmoothingFactor} \
            --out="${TXwFolder}/${TXwImage}_transplant"

        # reassigning variable names to transplanted image
        # leading to single input image for all subjects -> no anatomical averaging which can lead to distortion of focal lesion
        export TXwInputImages="${TXwFolder}/${TXwImage}_transplant.nii.gz"
    fi
	...
	${RUN} ${LEAPP_STRUCTDIR}/Scripts/PreFS/ACPCAlignment.sh \
	...
	${RUN} ${LEAPP_STRUCTDIR}/Scripts/PreFS/BrainExtraction_FNIRTbased.sh \
...
# INCLUDING: lesion mask for cost function masking -> using Atlas...costMask.sh script
${RUN} ${LEAPP_STRUCTDIR}/Scripts/PreFS/AtlasRegistrationToMNI152_FLIRTandFNIRT.sh \

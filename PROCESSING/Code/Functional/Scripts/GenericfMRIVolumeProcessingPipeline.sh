...
if [ -z ${Session} ]; then
	log_Err_Abort "--session must be specified"
fi
...
SubjectFolder="${Path}/${Subject}/${Session}"
...
if [ "$fMRIReference" = "NONE" ]; then
  ...
  fMRIReferencePath="${SubjectFolder}/${fMRIReference}"
  log_Msg "Using reference image from ${fMRIReferencePath}"
  fMRIReferenceImage="${fMRIReferencePath}"/"${ScoutName}"_gdc
  fMRIReferenceImageMask="${fMRIReferencePath}"/"${ScoutName}"_gdc_mask
  ReferenceResultsFolder="${SubjectFolder}/${AtlasSpaceFolder}/${ResultsFolder}/${fMRIReference}"
...
if [ $fMRIReference = "NONE" ] ; then
  ...
  ${RUN} ${LEAPP_FUNCTDIR}/Scripts/DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased.sh \
         --workingdir=${DCFolder} \
         --scoutin=${fMRIFolder}/${ScoutName}_gdc \
         --t1lesionmask="${Path}/${Subject}/${Session}/lesion/T1w_acpc_dc_restore_mask_invert.nii.gz" \

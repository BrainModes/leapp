...
#
# ## Adjusted script from HCP minimal processing pipeline (Glasser et al. (2013))
...
T1wCostMask=`getopt1 "--t1lesionmask" $@`
...
if [[ ! -f ${T1wCostMask} ]]; then
    log_Msg "WARNING:    Performing fMRI processing without lesion mask. Are you processing controls? If not please run StructuralPipeline.sh first."
fi
...
case $DistortionCorrection in
    ...
    ${FIELDMAP_METHOD_OPT} | ${SIEMENS_METHOD_OPT} | ${GENERAL_ELECTRIC_METHOD_OPT})
        ...
        if [[ -f ${T1wCostMask} ]]; then
            log_Msg "UPDATE:    Performing registration using cost function masking <<T1w_acpc_dc_restore_mask_invert.nii.gz>>"
            if [[ X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) > 1.25 ) {1}" | bc -l` = X1 || X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) < 0.75 ) {1}" | bc -l` = X1 ]] ; then
                ${FSLDIR}/bin/flirt -interp spline -dof 6 -in ${WD}/Magnitude.nii.gz -ref ${T1wImage} \
                -refweight ${T1wCostMask} \
                -omat "$WD"/Mag2T1w.mat -out ${WD}/Magnitude2T1w.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30
                ...
                # ${FSLDIR}/bin/flirt -interp spline -dof 6 -in ${WD}/Scout.nii.gz -ref ${T1wImage} \
                # -refweight ${T1wCostMask} \
                # -omat "$WD"/Scout2T1w.mat -out ${WD}/Scout2T1w.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30
                # ${FSLDIR}/bin/flirt -interp spline -cost mutualinfo -dof 6 -in ${WD}/Scout.nii.gz -ref ${T1wImage} \
                # -refweight ${T1wCostMask} \
                # -omat "$WD"/Scout2T1w.mat -out ${WD}/Scout2T1w.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30
                ${FSLDIR}/bin/flirt -interp spline -cost mutualinfo -dof 12 -in ${WD}/Scout.nii.gz -ref ${T1wImage} \
                -refweight ${T1wCostMask} \
                -omat "$WD"/Scout2T1w.mat -out ${WD}/Scout2T1w.nii.gz
                ...
                # register scout to T1w image using fieldmap
                ${HCPPIPEDIR_Global}/epi_reg_dof --dof=${dof} --epi=${WD}/Scout.nii.gz --t1=${T1wImage} --t1brain=${WD}/${T1wBrainImageFile} \
                --weight=${T1wCostMask} \
                --out=${WD}/${ScoutInputFile}${ScoutExtension} --fmap=${WD}/FieldMap.nii.gz --fmapmag=${WD}/Magnitude.nii.gz --fmapmagbrain=${WD}/Magnitude_brain.nii.gz --echospacing=${EchoSpacing} --pedir=${UnwarpDir}

            fi
        else
         log_Msg "UPDATE:    Performing registration without cost function masking."
            if [[ X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) > 1.25 ) {1}" | bc -l` = X1 || X`echo "if ( (${MagnitudeBrainSize} / ${T1wBrainSize}) < 0.75 ) {1}" | bc -l` = X1 ]] ; then
                ${FSLDIR}/bin/flirt -interp spline -dof 6 -in ${WD}/Magnitude.nii.gz -ref ${T1wImage} \
                -omat "$WD"/Mag2T1w.mat -out ${WD}/Magnitude2T1w.nii.gz -searchrx -30 30 -searchry -30 30 -searchrz -30 30
                ${FSLDIR}/bin/convert_xfm -omat "$WD"/T1w2Mag.mat -inverse "$WD"/Mag2T1w.mat
    ...
    ${SPIN_ECHO_METHOD_OPT})
        ...
        if [ $PreregisterTool = "epi_reg" ] ; then
          ...
          ${FSLDIR}/bin/flirt -in ${WD}/${ScoutInputFile}${ScoutExtension} -ref ${WD}/${T1wBrainImageFile} \
          -refweight ${T1wCostMask} \
          -out ${WD}/${ScoutInputFile}${ScoutExtension}2T1w_init -omat ${WD}/${ScoutInputFile}${ScoutExtension}2T1w_init.mat -dof ${dof}
        else
    ...
    ${NONE_METHOD_OPT})
            ...
            if [ "$PreregisterTool" = "epi_reg" ] ; then
                ...
                ${FSLDIR}/bin/flirt -in ${WD}/${ScoutInputFile}${ScoutExtension} -ref ${WD}/${T1wBrainImageFile} \
                -refweight ${T1wCostMask} \
                -out ${WD}/${ScoutInputFile}${ScoutExtension}2T1w_init -omat ${WD}/${ScoutInputFile}${ScoutExtension}2T1w_init.mat -dof 6
            fi
